class FuzzySet:
	...
	def create_trapezoidal(domain_min, domain_max, low, mid1, mid2, high):
		...
		t1fs._degree = [round(min(max(min((dom-low)/(mid1-low), (high-dom)/(high-mid2)), 0), 1), 2) for dom in t1fs._domain]
		return t1fs
	...
	def create_triangular(domain_min, domain_max, low, mid, high):
		...
		if mid == low:
			t1fs._degree = [round(max((high - dom)/(high - mid), 0),2) for dom in t1fs._domain]
		elif mid == high:
			t1fs._degree = [round(max((dom - low)/(mid - low), 0), 2) for dom in t1fs._domain]
		else:
			t1fs._degree = [round(max(min((dom - low)/(mid - low), (high - dom)/(high - mid)), 0), 2) for dom in t1fs._domain]
